...
func (m *Main) Run() error {
	...
	go func() {
		for range c {
			log.Printf("Rides: %d, Bytes: %s", m.nexter.Last(), pdk.Bytes(m.recordManager.BytesProcessed()))
			os.Exit(0)
		}
	}()
...
func (m *Main) printStats() *time.Ticker {
	...
	go func() {
		for range t.C {
			...
			bytes := m.recordManager.BytesProcessed()
			log.Printf("Rides: %d, Bytes: %s, Records: %v, Duration: %v, Rate: %v/s", m.nexter.Last(), pdk.Bytes(bytes), m.recordManager.totalRecs.Get(), duration, pdk.Bytes(float64(bytes)/duration.Seconds()))
			log.Printf("Skipped: %v, badLocs: %v, nullLocs: %v, badSpeeds: %v, badTotalAmnts: %v, badDurations: %v, badUnknowns: %v, badPassCounts: %v, badDist: %v", m.recordManager.skippedRecs.Get(), m.badLocs.Get(), m.nullLocs.Get(), m.badSpeeds.Get(), m.badTotalAmnts.Get(), m.badDurations.Get(), m.badUnknowns.Get(), m.badPassCounts.Get(), m.badDist.Get())
		}
	}()
	return t
}
...
Records:
	for record := range records {
		...
		if !ok {
			m.recordManager.skippedRecs.Add(1)
			continue
		}
